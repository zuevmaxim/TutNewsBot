...
class Subscription:
    def __init__(self, user_id, subscription, last_seen_post=-1):
...
def add_subscription(subscription):
    current = subscriptions_db.getBy({"user_id": subscription.user_id, "subscription": subscription.subscription})
    if len(current) == 0:
        subscriptions_db.add({"user_id": subscription.user_id, "subscription": subscription.subscription,
                              "last_seen_post": subscription.last_seen_post})


def remove_subscription(user_id, subscription):
    current = subscriptions_db.getBy({"user_id": user_id, "subscription": subscription})
    if len(current) == 1:
        current = current[0]
...
def get_subscription_names(user_id=None):
    ...
    return [e["subscription"] for e in current]
...
def update_last_seen_post(user_id, subscription, last_post):
    current = subscriptions_db.getBy({"user_id": user_id, "subscription": subscription})
    if len(current) == 1:
        current = current[0]
